      ...
      <div style="display: flex; align-items: flex-start">
        <div style="width: 200px; border: 1px solid #ddd; border-radius: 5px; height: calc(50vh + 125px);">
          <div style="padding: 10px; border-bottom: 1px solid #ddd; color: #000; background-color: #eee"
               v-if="messageType === 'admin' || messageType === 'all'" >管理员</div>
          <div class="user-list-box" v-bind:style="{ height: messageType === 'all' ? '15%' : '' }"
               v-if="messageType === 'admin' || messageType === 'all'">
            <div class="user-list-item" v-for="item in adminData" :key="item.id" @click="selectToUser(item)">
              <img :src="item.avatar" style="width: 30px; height: 30px; border-radius: 50%" alt="">
          ...
          <div style="padding: 10px; border-bottom: 1px solid #ddd; color: #000; background-color: #eee"
               v-if="messageType === 'homestay' || messageType === 'all'">民宿房主</div>
          <div class="user-list-box" v-bind:style="{ height: messageType === 'all' ? '31%' : '' }"
               v-if="messageType === 'homestay' || messageType === 'all'">
            <div class="user-list-item" v-for="item in homestayData" :key="item.id" @click="selectToUser(item)">
              <img :src="item.avatar" style="width: 30px; height: 30px; border-radius: 50%" alt="">
              <span style="flex: 1; margin-left: 10px;" :style="{ color: item.role+ '_' + item.name === toUser ? '#3a8ee6' : '' }">{{ item.name }}</span>
              <div class="user-list-item-badge" v-if="unRead?.[item.role + '_' + item.name]">{{ unRead?.[item.role + '_' + item.name] }}</div>
            </div>
          ...
          <div style="padding: 10px; border-bottom: 1px solid #ddd; color: #000; background-color: #eee"
               v-if="messageType === 'all'">民宿用户</div>
          <div class="user-list-box" v-bind:style="{ height: messageType === 'all' ? '28%' : '' }" style="overflow-y: scroll"
               v-if="messageType === 'all'">
            <div class="user-list-item" v-for="item in userData" :key="item.id" @click="selectToUser(item)">
              <img :src="item.avatar" style="width: 30px; height: 30px; border-radius: 50%" alt="">
              <span style="flex: 1; margin-left: 10px;" :style="{ color: item.role+ '_' + item.name === toUser ? '#3a8ee6' : '' }">{{ item.name }}</span>
              <div class="user-list-item-badge" v-if="unRead?.[item.role + '_' + item.name]">{{ unRead?.[item.role + '_' + item.name] }}</div>
            </div>
        ...
        <div style="width: 50%; border: 1px solid #ddd; border-radius: 5px; background-color: #f1f1f1; margin: 0 10px;">
          ...
          <div style="padding: 10px 5px; border-top: 1px solid #ddd; display: flex; align-items: center; width: 100%;">
            ...
            <div style="margin-left: 5px">
              <el-upload :action="this.$baseUrl + '/files/upload'"
...
export default {
  ...
  mounted() {
    ...
    //console.log('WebSocket连接地址:', client)
    client.onopen = () => {
      console.log('WebSocket连接已建立')
      this.$notify.success('WebSocket连接已建立')
    }
    ...
    // 根据消息类型加载对应的消息数据
    if (this.messageType === 'all') {// 加载所有消息数据

// 查询民宿房主的数据
      this.$request.get('/homestay/selectAllOK').then(res => {
        if (res.code === '200') {
          this.homestayData = res.data.filter(v => (v.role + '_' + v.name) !== this.fromUser);
        } else {
...
// 查询管理员的数据
      this.$request.get('/admin/selectAll').then(res => {
        if (res.code === '200') {
          this.adminData = res.data.filter(v => (v.role + '_' + v.name) !== this.fromUser);
        } else {
          this.$message.error(res.msg);
        }
      });
...
// 查询用户的数据
      this.$request.get('/user/selectAll').then(res => {
        if (res.code === '200') {
          this.userData = res.data.filter(v => (v.role + '_' + v.name) !== this.fromUser);
        } else {
          this.$message.error(res.msg);
        }
      });

      console.log('加载所有消息数据');
    }
    else if (this.messageType === 'admin') {
      // 加载管理员消息数据
      // 查询管理员的数据
      this.$request.get('/admin/selectAll').then(res => {
        if (res.code === '200') {
      ...
      console.log('加载管理员消息数据');
    }
    else if (this.messageType === 'homestay') {
      // 加载民宿房主消息数据
      // 查询民宿房主的数据
      this.$request.get('/homestay/selectAllOK').then(res => {
        if (res.code === '200') {
  ...
  methods: {
    load() {
      this.$request.get('/imSingle?fromUser=' + this.fromUser + '&toUser=' + this.toUser).then(res => {
        if (res.code === '200') {
    ...
    loadUnReadNums() {
      // 查询未读数量
      this.$request.get('/imSingle/unReadNums?toUsername=' + this.fromUser).then(res => {
      ...
      if (client.readyState === WebSocket.OPEN) {
        ...
        setTimeout(() => {
          if (client.readyState === WebSocket.OPEN) {
            ...
            this.$notify.error('WebSocket连接未建立或已关闭')
          }
        }, 1000) // 延迟1秒发送数据
      } else {
        // WebSocket处于关闭状态
