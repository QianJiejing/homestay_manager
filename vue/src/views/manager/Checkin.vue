<template>
  <div>
    ...
    <div class="operation">
      <el-button type="primary" plain @click="handleAdd" v-if="user.role === 'HOMESTAY'">入住登记</el-button>
    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="predictedOutTime" label="预退房时间" show-overflow-tooltip></el-table-column>
      ...
      <el-form :model="form" label-width="100px" style="padding-right: 50px" :rules="rules" ref="formRef">
        <el-form-item label="选择订单" prop="orderId">
          <el-select v-model="form.orderId" placeholder="请选择订单" style="width: 100%" @change="loadRoomData(form.orderId)">
            <el-option v-for="item in ordersData" :label="item.orderId + ' - ' + item.userName + ' - ' + item.typeName+ '  - 入住: ' + item.inTime" :value="item.orderId" :key="item.id"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="分配客房" prop="roomId">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        roomId: [
          {required: true, message: '请选择客房', trigger: 'blur'},
        ],
      },
    }
  },
  ...
  methods: {
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      let ITime = this.ordersData.find(item => item.orderId === this.form.orderId).inTime;
      if (this.form.inTime !== ITime) {
        this.$message.warning('请检查入住时间是否与订单一致');
        return;
      }
