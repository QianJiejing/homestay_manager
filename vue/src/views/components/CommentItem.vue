  ...
  <div>
    ...
    <div v-for="item in commentData" :key="item.id" :style="getItemStyle(item)">
      ...
      <div style="display: flex;" v-if=" item.children && item.children.length > 0">
        ...
        <el-col :span="22">
          <!-- 递归回复的回复 -->
          <div v-for="(reply, replyIndex) in flattenReplies(item)" :key="reply.id" :style="getItemStyle(reply)" >
            <div style="display: flex;" v-if="showAllReplies[item.id] || replyIndex < 2">
              <el-col :span="1.5" style="display: flex;margin-right: 5px">
          ...
          <div class="hover-effect" @click="toggleReplies(item.id)" v-if="item.replyCount > 2">
            <span v-if="!showAllReplies[item.id]">共 {{ item.replyCount - 2 }} 条回复，点击查看</span>
    ...
    flattenReplies(comment) {
      ...
      if (comment.children && comment.children.length > 0) {
        comment.children.forEach(child => {
          ...
          result = result.concat(this.flattenReplies(child));
        });
      }
      return result;
    },
