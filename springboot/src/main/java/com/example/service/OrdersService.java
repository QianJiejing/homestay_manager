...
public class OrdersService {
    ...
    public void add(Orders orders) {
        // 查询相应民宿是否有审核通过
        Integer homestayId = orders.getHomestayId();
        Homestay homestay = homestayMapper.selectById(homestayId);
        String status = homestay.getStatus();
        if(status.equals(StatusEnum.CHECKING_OK.status)) {
            ...
            try {
                ...
                // 根据入住时间和离开时间 计算天数
                Date start = dateFormat.parse(orders.getInTime());
                Date end = dateFormat.parse(orders.getOutTime());
                // 比较入住日期是否小于当前日期
/*                if (start.compareTo(currentDate) < 0) {
    ...
    public PageInfo<Orders> selectPage(Orders orders, Integer pageNum, Integer pageSize) {
        ...
        if (RoleEnum.HOMESTAY.name().equals(currentUser.getRole())) {
            orders.setHomestayId(currentUser.getId());
        }
    ...
    public List<Orders> selectByCheckingAndHomestayId(Integer id) {
        Orders orders = new Orders();
        orders.setHomestayId(id);
        orders.setStatus(OrdersEnum.STATUS_CHECKING.status);
        return ordersMapper.selectAll(orders);
    }
}